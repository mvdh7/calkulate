...
kwargs_core = dict(temperature=temperature, salinity=salinity, opt_pH_scale=3)
...
emf = calk.convert.h_to_emf(
    10.0 ** -pH_titrations, emf0, kwargs_titration["temperature"]
)
file_name = "tests/data/test_simulate_then_solve.dat"
calk.to_dat(
    pd.DataFrame(
        {
            "titrant_amount": titrant_mass * 1000,  # g
            "measurement": emf,  # mV
            "temperature": co2sys_titrations["temperature"],  # °C
        }
    ),
    file_name,
    mode="w",
...
ds = calk.Dataset(ds)
ds.prepare().solve()
co2sys_core["alkalinity_titration"] = ds.alkalinity.to_numpy()
