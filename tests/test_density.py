...
def test_HCl_NaCl_DSC07():
    ...
    rho_mix = calk.density.HCl_NaCl_25C_DSC07(
        molinity_HCl=0.2, molinity_NaCl=0.5
    )
    rho_mix = np.round(rho_mix, decimals=5)
    rho_DSC07 = 1.02056  # their check value
    assert np.round(rho_mix, 2) == np.round(rho_DSC07, 2)
    print(
        rho_mix, rho_DSC07
    )  # check fails for now... sent email to Andrew Dickson
