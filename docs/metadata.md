...
A Calkulate `Dataset` is a [pandas DataFrame](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.html) with [some extra methods added](../methods).

A `Dataset` can be made [from an existing DataFrame](../read/#convert-from-a-dataframe) or by [importing a spreadsheet from various formats](../read/#import-from-excel-csv-or-dbs).
...
??? info "`file_type` : *which file format are your titration data in?*"
    ...
    * `"genfromtxt"` : the [default VINDTA-style format](read.md/#their-default-format), or
...
??? info "`solve_mode` : *how to solve for alkalinity*"
    ...
    * `"pH_adjust"`: use least-squares fitting for titration data given in pH, but allowing the pH to be adjusted as if its EMF<sup>0</sup> was not perfectly calibrated.
